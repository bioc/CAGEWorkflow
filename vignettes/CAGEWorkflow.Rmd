...
The data is aquired via the `nanotubes` data package:
...
`CAGEfightR` starts analysis from CTSSs, which is the number of CAGE tag 5'-ends mapping to each basepair in the genome. CTSSs are normally stored as either BED-files or BigWig-files. `CAGEfightR` works on BigWig-files, since these can be efficiently imported and allow for random access.
...
`CAGEfightR` finds clusters by calcuting the pooled CTSS signal across all samples: We first normalize CTSSs count in each sample to Tags-Per-Million (TPM) values, and them sum TPM values across samples: 
...
Starting with the TSS candidates, we can not only annotate a TSS with overlapping transcripts, but also in what _part_ of a transcript a TSS lies by using a hierachical annotation scheme. As some TSSs might be very wide, we only use the TSS peak for annotation purposes:
...
```{r simpleTSS, fig.width=5, fig.height=5}
...
```{r simpleEnhancer, fig.width=5, fig.height=5}
...
```{r plotTxTypes, fig.width=5, fig.height=3}
# Number of clusters
...
ggplot(cluster_info, aes(x=txType, y=log2(score/ncol(RSE)), fill=clusterType)) +
...
```{r TSSShapes, fig.width=5, fig.height=3}
highTSSs %>%
	rowData %>%
	as.data.frame %>%
	ggplot(aes(x=IQR)) +
...
We can now investigate the core promoters sequences of the two classes of TSSs. We first need to extract the sequences for each TSS: We defince this as the TSS peak -40/+10 bp and extract them from using the `r BiocStyle::Biocpkg("BSgenome.Mmusculus.UCSC.mm10")` genome package:
...
This returns a `DNAStringSet`-object which we can plot as a sequence logo [@Manetti2015] via the `r BiocStyle::CRANpkg("ggseqplot")` package[@Wagih2017]:
```{r ggseqlogo, fig.width=5, fig.height=2.5}
promoter_seqs %>%
	as.character %>%
...
```{r browseLinks, fig.width=5, fig.height=5}
# Make plotting region
...
Several studies have found that groups or stretches of closely spaced enhancers tend to show different chromatin characteristics and functions compared to singleton enhancers. Such groups of are often refered to as "super enhancers" or "stretch enhancers"[@Pott2015].
...
```{r findStretches, fig.width=5, fig.height=5}
...
The returned `GRanges` contains the the location, number of enhancers and average correlation for each stretch. Stretches are found in a variatity of context, some being intergenic and other spanning various parts of genes. Let us plot one of the top intergenic stretches:

```{r browseStretches, fig.width=5, fig.height=5}
# Make plotting region
...
res %>%
    top_n(-10, padj) %>%
    dplyr::select(cluster=row, clusterType, txType, baseMean, log2FoldChange, padj) %>%
    kable
```

It always a good idea to inspect a few diagnostics plot to make sure the `DESeq2` analysis was succesful. One such example is an MA-plot (another useful plot is p-value histogram):

```{r MAplot, fig.width=5, fig.height=5}
...
```{r correctedPCA, fig.width=4, fig.height=4}
# Overwrite assay 
...
```{r ploTop10, fig.width=6, fig.height=5}
...
table(Hnf4alpha=motifMatches(motif_hits)[,"MA0099.2"],
      DE=res$padj<0.05) %>%
    print() %>%
    fisher.test()
```
...
The geneIDs in this case is Entrez ID (which is widely used by Bioconductor packages). We can translate these systematic IDs into more human-readable symbols using the `r BiocStyle::Biocpkg("org.Mm.eg.db")` annotation package:
...
Similarly to the `DESeq2` analysis, we first build the necesarry object and then normalize the expression values:
...
```{r pathview, message = FALSE, fig.width=6, fig.height=6}
# Visualize a KEGG
...
```{r TSSstructure, fig.width=5, fig.height=2.5}
RSE_filtered %>% 
    rowData %>% 
    as.data.frame %>% 
    as_tibble %>% 
...
```{r heatmap, fig.width=3, fig.height=4}
RSE_filtered %>%
	subset(geneID == "74202") %>%
	assay("ComBat") %>%
	t %>%
...
```{r dtubrowser, fig.width=5, fig.height=5}
# Define plot area
